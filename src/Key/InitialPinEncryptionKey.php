...
/**
 * Class InitialPinEncryptionKey
 *
 * @author Vitalii Ekert <vitalii.ekert@tonicforhealth.com>
 */
class InitialPinEncryptionKey extends AbstractKey
{
    ...
    protected function validateKey($binKey)
    {
        if (self::KEY_SIZE_BYTES !== ($keySize = strlen($binKey))) {
            throw new \InvalidArgumentException(
                sprintf('Invalid IPEK size (expected %d bytes, got %d).', self::KEY_SIZE_BYTES, $keySize)
            );
        }
